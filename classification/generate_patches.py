...
if __name__ == "__main__":
    ...
    case = os.path.splitext(os.path.basename(wsi_file))[0]
    det_xml = os.path.join(case_dir, case,  f'{case}.xml')

    try:
        simg = openslide.open_slide(wsi_file)

    except:
        print('%s', case)
        exit()

    output_dir = os.path.join(case_dir, case , 'patch')
    ...
    with open(os.path.join(case_dir,case,'patch.json'), 'w') as f:
