...
def scn_to_patchs(scn_file, working_dir, opt):
    ...
    if opt.lv >= 2:
        ...
        img_lv2 = simg.read_region((start_x, start_y), opt.lv, (img_lv2_width, img_lv2_height))
        # img_lv2 = simg.read_region((start_x, start_y), opt.lv, (10707,3027))
...
def run_one_scn(demo_scn,demo_dir,opt):
    ...
    for di in range(len(detect_all)):
        ...
        detect_dict['@Text'] = ('%.3f' % detect_one[3]) if opt.nontext == False else ''
        ...
        if 'leica.device-model' in simg.properties:  #leica
            coord1 = {}
            coord1['@Y'] = str(height_y - (detect_one[1] - detect_one[2]) * down_rate)
            coord1['@X'] = str((detect_one[0] - detect_one[2]) * down_rate)
            coord1['@Z'] = '0'
            coord2 = {}
            coord2['@Y'] = str(height_y - (detect_one[1] + detect_one[2]) * down_rate)  # 左右
            coord2['@X'] = str((detect_one[0] + detect_one[2]) * down_rate)  # 上下
            coord2['@Z'] = '0'
            ...
            coord1['@Z'] = '0'
            coord2 = {}
            coord2['@Y'] = str((detect_one[1] + detect_one[2]) * down_rate)  # 左右
            coord2['@X'] = str((detect_one[0] + detect_one[2]) * down_rate)  # 上下
            coord2['@Z'] = '0'
            detect_dict['Vertices']['Vertex'].append(coord1)
            detect_dict['Vertices']['Vertex'].append(coord2)
            ...
            # coord1['@X'] = str((detect_one[1] - detect_one[2]) * down_rate)
            # coord1['@Z'] = '0'
            # coord2 = {}
            # coord2['@Y'] = str((detect_one[0] + detect_one[2]) * down_rate)  # 左右
            # coord2['@X'] = str((detect_one[1] + detect_one[2]) * down_rate)  # 上下
            # coord2['@Z'] = '0'
            # detect_dict['Vertices']['Vertex'].append(coord1)
            # detect_dict['Vertices']['Vertex'].append(coord2)
